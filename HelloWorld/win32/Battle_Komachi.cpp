...
Battle_Komachi::Battle_Komachi(int cTag)
{
	init();
	...
	setTag(cTag);
	this->autorelease();
	this->setResourceWithString("komachi");
	m_iCharacterName = CN_HERO_KOMACHI;
...
void Battle_Komachi::doCard(const Action_Card &card)
{
	...
	switch (card.getCardNumber())
	{
		...
		case AC_BACKSTEP:
		{
							m_listOrder.push_back(kwOrderMoveToPoint::makekwOrderMoveToPoint(
								this,
								moveBackPoint
								, 100.f, 400.f, 200.f, shared_ptr<kwMsg_Ani>(new kwMsg_Ani(CAT_RUN, 0, 2, 0, 0.1f))));
		...
		{					
						   m_listOrder.push_back(
							   ...
							   shared_ptr<kwMsg_Action>(
							   new kwMsg_Action(
							   new kwMoving_Vector(this, 400.f, CG3Point(x, 0, 0)),
							   m_pMap->getCurrentTargetPointer(),
							   50,
							   combineInt(1, 0),
							   1.f,
							   ...
							   shared_ptr<kwMsg_Ani>(new kwMsg_Ani(CAT_ATTACK5, 0, 0, 0, 0.02f))
							   ));
						   break;
		}
		case AC_NOCARD_BACK:
